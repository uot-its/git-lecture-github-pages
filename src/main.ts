...
const aiReviewTools: AiReviewTool[] = [
  {
...
function section(id: string, label: string, title: string, body: string, children: Node[] = []): HTMLElement {
...
function renderHero(): HTMLElement {
  ...
  left.append(
    el('p', { class: 'eyebrow' }, ['NEW ENGINEER TRAINING // GITHUB']),
    el('h1', { class: 'glitch', 'data-text': 'GitHub研修' }, ['GitHub研修']),
    el('p', { class: 'hero-lead' }, ['GitHubを知らない状態から, 安全にチーム開発へ参加できる状態まで進む. コマンド暗記ではなく, 変更を小さくし, 意図を残し, PRで合意する型を身につける.']),
...
function renderVocabulary(): HTMLElement {
...
function renderThreeStates(): HTMLElement {
...
function renderEmergency(): HTMLElement {
...
function renderExercises(): HTMLElement {
...
function renderChecklist(): HTMLElement {
  const saved = new Set(JSON.parse(localStorage.getItem('github-training-checks') ?? '[]') as number[]);
  const list = el('div', { class: 'checklist' }, checklist.map((item, index) => {
    const input = el('input', { type: 'checkbox', id: `check-${index}` }) as HTMLInputElement;
    input.checked = saved.has(index);
    input.addEventListener('change', () => {
      const current = new Set(JSON.parse(localStorage.getItem('github-training-checks') ?? '[]') as number[]);
...
function renderClosing(): HTMLElement {
...
function updateProgress(): void {
  const checked = document.querySelectorAll<HTMLInputElement>('.check-item input:checked').length;
  const ratio = Math.round((checked / checklist.length) * 100);
  const label = document.getElementById('completion-label');
  const fill = document.getElementById('completion-fill') as HTMLElement | null;
  if (label) label.textContent = `${ratio}% complete`;
...
function setupInteractions(): void {
  document.addEventListener('click', async (event) => {
    ...
    const copyButton = target.closest<HTMLButtonElement>('.copy');
    if (copyButton) {
    ...
    const choice = target.closest<HTMLButtonElement>('.choice');
    if (choice) {
      ...
      if (cIndex === item.answer) choice.classList.add('correct');
      else choice.classList.add('wrong');
      const explain = document.getElementById(`quiz-explain-${qIndex}`);
      if (explain) explain.textContent = item.explain;
    }
  });
  ...
  const navSystem = document.querySelector<HTMLElement>('.nav-system');
